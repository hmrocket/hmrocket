...
if [ -f $MANIFEST ]
then
    LINE=$(grep -o ${MANIFEST} -e 'versionCode [0-9]*');
	declare -a LINE;
	LINE=(`echo $LINE | tr "\"" " "`);
	VERSION=(${LINE[1]});
	INCREMENTED=$(($VERSION+1))
    sed "s/versionCode ${INCREMENTED}/" $MANIFEST > $MANIFEST.tmp && mv $MANIFEST.tmp $MANIFEST
    git add $MANIFEST
...
if [ -f $MANIFEST ]
then
    LINE=$(grep -o ${MANIFEST} -e 'versionName "^[0-9]+(\.\d+)?"');
	declare -a LINE;
	LINE=(`echo $LINE | tr "\"" " "`);
	VERSION=(${LINE[1]});
	INCREMENTED=$(($VERSION+1))
	...
	sed "s/versionName \"${INCREMENTED}\"/" $MANIFEST > $MANIFEST.tmp && mv $MANIFEST.tmp $MANIFEST
	git add $MANIFEST
	echo "Version Name update to ${INCREMENTED} In ${MANIFEST}!";
	versionName=INCREMENTED
fi
